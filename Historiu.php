<?php
$sqlall = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]'";
...
$sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' ORDER BY `Tanggal` DESC LIMIT $_SESSION[hal]";
if(isset($_GET['masuk'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND (`Keterangan`='Hadir' OR `Keterangan`='Terlambat') ORDER BY `Tanggal` DESC";
} else if(isset($_GET['tdkmasuk'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND (`Keterangan`='Ijin' OR `Keterangan`='Sakit' OR `Keterangan`='Alpa') ORDER BY `Tanggal` DESC";
} else if(isset($_GET['hadir'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND `Keterangan`='Hadir' ORDER BY `Tanggal` DESC";
} else if(isset($_GET['ijin'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND `Keterangan`='Ijin' ORDER BY `Tanggal` DESC";
} else if(isset($_GET['sakit'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND `Keterangan`='Sakit' ORDER BY `Tanggal` DESC";
} else if(isset($_GET['terlambat'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND `Keterangan`='Terlambat' ORDER BY `Tanggal` DESC";
} else if(isset($_GET['alpa'])) {
  $sql = "SELECT * FROM `absen` WHERE `Nama`='$_SESSION[Username]' AND `Keterangan`='Alpa' ORDER BY `Tanggal` DESC";
}
...
<body>
  ...
  <div class="card m-3">
    <div class="card-body m-3">
      <h4 class="card-title" style="color:#0891C0">Histori Pribadi</h4>
      <div class="dropdown">
      ...
      <table border="1" style="text-align:center;" class="w-100">
        ...
        if (mysqli_num_rows($result)>0) {
          while($row = $result->fetch_assoc()) {
            ...
            <?php
            // $row["Tanggal"]." ⠀ ".$row["WaktuDatang"]."-".$row["WaktuPulang"]." ⠀ ".$row["Groupname"]." - ".$row["Nama"]." ".$row["Keterangan"]."<br>";
          }
        } ?>
      </table><?php
